...
class UserManagementService:
    ...
    def update_user_profile(self, user_id: int, profile_data: dict) -> dict:
        """Update user profile information."""
        # Update user profile
        updated_user = self.user_repository.update_user(user_id, profile_data)
        if not updated_user:
            raise ValueError(f"User with ID {user_id} not found")
        return updated_user

    def assign_user_role(self, user_id: int, role: str) -> bool:
        """Assign role to user."""
        # Validate role
        ...
        # Update user role
        success = self.user_repository.update_user_role(user_id, role)
        if not success:
            raise ValueError(f"User with ID {user_id} not found")
        return success
