...
class QueryRepository:
    ...
    async def get_by_conversation_id(self, conversation_id: uuid.UUID) -> List[Query]:
        """Get queries by conversation ID."""
        try:
            stmt = select(Query).where(Query.conversation_id == conversation_id)
            result = await self.session.execute(stmt)
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Failed to get queries for conversation {conversation_id}: {str(e)}")
            raise DatabaseError(f"Failed to get queries for conversation: {str(e)}")
